...
public class FileUtils {
    ...
    public static StringBuffer readFileByLine(String dirPath) {
        ...
        if (!file.exists()) {
            ...
            try {
                ...
                byte[] byt = new byte[3];
                input.read(byt);
                String encodeType = EncodeTypeEnum.GBK.getCode();
                if (byt[0] == -17 && byt[1] == -69 && byt[2] == -65) {
                    encodeType = EncodeTypeEnum.UTF8.getCode();
                }
                //FileReader fileReader = new FileReader(file);
                InputStreamReader fileReader = new InputStreamReader(new FileInputStream(file), encodeType);
     ...
     * @return
     */
    public String getFileEncodeType(byte[] byt) {
